...
export const MINIMUM_MEETING_NOTICE_MINUTES = 30;
...
export function validateMeetingSettings(source = {}) {
  ...
  if (settings.meeting_min_notice_minutes < MINIMUM_MEETING_NOTICE_MINUTES || settings.meeting_min_notice_minutes > 10080) errors.meeting_min_notice_minutes = "Minimum notice must be between 30 minutes and seven days.";
  ...
  if (![15, 30, 60].includes(settings.meeting_slot_interval_minutes)) errors.meeting_slot_interval_minutes = "Choose a 15, 30, or 60 minute slot interval.";
...
export function buildMeetingSlots({
  ...
  for (let offset = 0; offset <= settings.meeting_max_days_ahead; offset += 1) {
    ...
    const matchingHours = hours.filter((row) => Number(row.day_of_week) === dayOfWeek(baseDateKey) && !row.is_closed);
    for (const row of matchingHours) {
      ...
      for (let slotMinutes = open; slotMinutes + settings.meeting_duration_minutes <= endOfWindow; slotMinutes += settings.meeting_slot_interval_minutes) {
        ...
        if (startMillis < earliest || startMillis > latest || intervals.some((busy) => startMillis < busy.end && endMillis > busy.start)) continue;
...
export function isMeetingStartBookable(startAt, { now = new Date(), minimumNoticeMinutes = MINIMUM_MEETING_NOTICE_MINUTES } = {}) {
  const startMillis = new Date(startAt || "").getTime();
