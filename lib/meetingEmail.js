...
export async function sendMeetingNotification({ admin, call, eventType }) {
  ...
  const hasDirectMeetingLink = Boolean(meetingTime) && !isCancelled && !isRescheduleRequest;
  const actionUrl = hasDirectMeetingLink
    ? new URL(`/meeting/${encodeURIComponent(selectedCall.id)}`, appUrl)
    : new URL(customer ? "/messages" : "/owner/calendar", appUrl);
  const actionLabel = canJoinDirectly ? "Open secure meeting" : hasDirectMeetingLink ? "View meeting request" : "Open meeting details";
  ...
  const timeDetails = eventType === "RESCHEDULED"
