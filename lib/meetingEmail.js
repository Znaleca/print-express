...
export async function sendMeetingNotification({ admin, call, eventType }) {
  ...
  const actionUrl = canJoinDirectly
      ...
      <div style="background:#f6f6f2;padding:32px;font-family:Arial,sans-serif;color:#1a1a1a;">
        <div style="max-width:560px;margin:auto;background:#fff;border:1px solid #d8d6ce;">
          ...
          <div style="padding:28px;">
            ...
            <a href="${safeActionUrl}" style="display:inline-block;background:#1a1a1a;color:#fff;text-decoration:none;padding:13px 18px;font-weight:700;">${canJoinDirectly ? "Join secure meeting" : "Open meeting details"}</a>
            ${canJoinDirectly ? '<p style="margin:14px 0 0;color:#555;font-size:12px;line-height:1.6;">The secure call opens 15 minutes before the scheduled time. Sign in with the email address that received this invitation.</p>' : ""}
          </div>
        </div>
      </div>
    `,
