...
async function getReviewPayload(supabase, ownerId) {
  ...
  const visibleReviews = reviews.filter((review) => !review.feedback_hidden);
  const hiddenReviews = reviews.filter((review) => review.feedback_hidden);
  const { data: requestRows, error: requestError } = rows.length > 0
    ? await supabase
      .from("review_moderation_requests")
      .select("id, order_id, business_id, reason, status, admin_response, reviewed_by, reviewed_at, created_at, updated_at")
      .eq("business_id", business.id)
  ...
  const serviceIds = [...new Set(rows.map((row) => row.review_service_id).filter(Boolean))];
  ...
  return {
    business,
    reviews: visibleDecoratedReviews.map((review) => ({ ...review, moderationRequests: requestsByOrder[review.order_id] || [] })),
    hiddenReviews: hiddenDecoratedReviews.map((review) => ({ ...review, moderationRequests: requestsByOrder[review.order_id] || [] })),
    ...
    summary: {
      ...
      averageRating: average(visibleReviews),
      overallAverageRating: average(reviews),
...
export async function POST(request) {
  try {
    ...
    if (!orderId || reason.length < 5 || reason.length > 1000) {
      return NextResponse.json({ error: "A reason of 5 to 1000 characters is required." }, { status: 400 });
    }

    const { data, error } = await auth.supabase.rpc("request_review_removal", {
      p_order_id: orderId,
      p_reason: reason,
