...
import { CalendarDays, ChevronLeft, ChevronRight, Clock3, Loader2, Mail, MessageSquare, RefreshCcw, Save, Video, XCircle } from "lucide-react";
...
export default function OwnerCalendarPage() {
  ...
  const previewSlots = useMemo(() => buildMeetingSlots({ timeZone, hours: hoursDraft, settings: settingsDraft || {}, bookedCalls: calls }), [timeZone, hoursDraft, settingsDraft, calls]);
  ...
  const updateSetting = (key, value) => setSettingsDraft((current) => ({ ...current, [key]: ["meeting_enabled", "meeting_requires_approval"].includes(key) ? value : Number(value) }));
  ...
  const saveAvailability = async () => {
    ...
    try {
      const { error: businessError } = await supabase.from("businesses").update({
        ...
        meeting_requires_approval: settingsDraft.meeting_requires_approval,
      }).eq("id", business.id);
  ...
  return (
    <div className="min-h-full bg-[#F6F6F2] px-4 py-6 text-slate-900 sm:px-6 lg:px-8">
        ...
        {loading && !business ? <div className="flex min-h-72 items-center justify-center gap-2 text-sm font-semibold text-slate-500"><Loader2 size={18} className="animate-spin" /> Loading calendar…</div> : !business ? <div className="rounded-xl border border-slate-200 bg-white p-8 text-center text-sm font-semibold text-slate-600">Create or verify a shop before publishing meeting availability.</div> : (
          <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
            <section className="rounded-2xl border border-[#D8D6CE] bg-white p-4 shadow-sm sm:p-5">
              ...
              <div className="mt-4 grid grid-cols-7 gap-1 text-center text-[10px] font-black uppercase tracking-wide text-slate-400">{DAY_NAMES.map((day) => <span key={day} className="py-2">{day.slice(0, 3)}</span>)}{cells.map((cell) => { const count = callsByDate.get(cell.dateKey)?.length || 0; return <button type="button" key={cell.dateKey} onClick={() => setSelectedDate(cell.dateKey)} className={`min-h-16 rounded-lg border p-2 text-left transition-colors ${cell.inMonth ? "bg-white" : "bg-slate-50 text-slate-300"} ${selectedDate === cell.dateKey ? "border-[#EC008C] ring-2 ring-[#EC008C]/15" : "border-slate-100 hover:border-slate-300"}`}><span className="text-xs font-bold">{cell.date.getUTCDate()}</span>{count > 0 && <span className="mt-2 flex w-fit items-center gap-1 rounded-full bg-[#00FFFF] px-1.5 py-0.5 text-[9px] font-black text-slate-900">{count}</span>}</button>; })}</div>
              ...
              <div className="mt-3 space-y-3">{selectedCalls.length ? selectedCalls.map((call) => { const callWindow = getVideoCallWindow(call); const start = call.scheduled_at || call.requested_slot_at; const isMissed = call.status === "EXPIRED" || (call.status === "SCHEDULED" && callWindow.expired); return <article key={call.id} className="rounded-xl border border-slate-200 bg-[#F6F6F2] p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><p className="font-black text-slate-900">{call.customer?.full_name || "Customer"}</p><span className={`rounded-full px-2 py-1 text-[10px] font-black ${call.status === "CANCELLED" ? "bg-rose-100 text-rose-700" : call.status === "REQUESTED" ? "bg-amber-100 text-amber-800" : isMissed ? "bg-slate-200 text-slate-700" : "bg-emerald-100 text-emerald-800"}`}>{isMissed ? "Missed / expired" : getMeetingStatusLabel(call.status)}</span></div><p className="mt-1 text-xs font-semibold text-slate-700">{start ? formatMeetingDateTime(start, call.timezone || timeZone) : "Time to be confirmed"}</p><p className="mt-1 text-[11px] text-slate-500">{call.duration_minutes || 30} minutes · {call.timezone || timeZone}</p>{call.status === "SCHEDULED" && !isMissed && <p className={`mt-2 inline-flex items-center gap-1 text-[10px] font-bold ${call.reminder?.status === "SENT" ? "text-emerald-700" : call.reminder?.status === "FAILED" ? "text-rose-700" : "text-slate-500"}`}><Mail size={12} />{call.reminder?.status === "SENT" ? "15-minute reminder sent" : call.reminder?.status === "FAILED" ? "Reminder will retry" : "15-minute reminder scheduled"}</p>}{isMissed && <p className="mt-2 text-[10px] font-bold text-slate-600">The meeting window has ended. Ask the customer to choose another time.</p>}{call.customer_note && <p className="mt-3 border-l-2 border-[#FFF200] pl-3 text-xs text-slate-600">{call.customer_note}</p>}</div><div className="flex flex-wrap gap-2 sm:justify-end"><Link href={`/owner/messages?conversation=${encodeURIComponent(call.conversation_id)}`} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-[10px] font-bold text-slate-700 hover:border-slate-400"><MessageSquare size={13} /> Conversation</Link>{call.status === "REQUESTED" && <button type="button" onClick={() => { setSelectedCall(call); setShowMeetingBooking(true); }} className="rounded-lg bg-[#EC008C] px-2.5 py-2 text-[10px] font-bold text-white">Confirm / choose time</button>}{["SCHEDULED", "LIVE"].includes(call.status) && <button type="button" onClick={() => joinCall(call.id)} disabled={!callWindow.joinable} className="inline-flex items-center gap-1 rounded-lg bg-slate-900 px-2.5 py-2 text-[10px] font-bold text-white disabled:cursor-not-allowed disabled:opacity-40"><Video size={13} />{callWindow.joinable ? "Join" : isMissed ? "Missed" : "Not ready"}</button>}{isMissed && <button type="button" onClick={() => requestReschedule(call.id)} className="rounded-lg bg-[#EC008C] px-2.5 py-2 text-[10px] font-bold text-white hover:bg-[#c90078]">Ask customer to reschedule</button>}{["REQUESTED", "SCHEDULED"].includes(call.status) && !isMissed && <><button type="button" onClick={() => { setSelectedCall(call); setShowMeetingBooking(true); }} className="rounded-lg border border-slate-300 bg-white px-2.5 py-2 text-[10px] font-bold text-slate-700">Reschedule</button><button type="button" onClick={() => cancelCall(call.id)} className="inline-flex items-center gap-1 rounded-lg border border-rose-200 bg-white px-2.5 py-2 text-[10px] font-bold text-rose-700"><XCircle size={13} /> Cancel</button></>}</div></div></article>; }) : <div className="rounded-xl border border-dashed border-slate-300 bg-[#F6F6F2] p-8 text-center text-sm text-slate-500">No meetings for this day.</div>}</div>
            </section>

            <aside className="space-y-5">
              <section className="rounded-2xl border border-[#D8D6CE] bg-white p-4 shadow-sm sm:p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#EC008C]">Availability</p><h2 className="mt-1 text-xl font-black">Booking settings</h2></div><Clock3 className="text-[#00aeb5]" size={20} /></div><p className="mt-2 text-xs leading-relaxed text-slate-500">These settings control the open slots customers can book. Confirmed meetings keep their saved settings. Server time always requires at least 30 minutes&apos; notice.</p><div className="mt-4 grid grid-cols-2 gap-3"><label className="text-xs font-bold text-slate-700">Duration<select value={settingsDraft?.meeting_duration_minutes || 30} onChange={(event) => updateSetting("meeting_duration_minutes", event.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-2 text-xs"><option value="15">15 min</option><option value="30">30 min</option><option value="45">45 min</option><option value="60">60 min</option></select></label><label className="text-xs font-bold text-slate-700">Slot interval<select value={settingsDraft?.meeting_slot_interval_minutes || 30} onChange={(event) => updateSetting("meeting_slot_interval_minutes", event.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-2 text-xs"><option value="15">15 min</option><option value="30">30 min</option><option value="60">60 min</option></select></label><label className="text-xs font-bold text-slate-700">Buffer<select value={settingsDraft?.meeting_buffer_minutes || 0} onChange={(event) => updateSetting("meeting_buffer_minutes", event.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-2 text-xs"><option value="0">No buffer</option><option value="15">15 min</option><option value="30">30 min</option><option value="60">60 min</option></select></label><label className="text-xs font-bold text-slate-700">Min notice<select value={settingsDraft?.meeting_min_notice_minutes || 30} onChange={(event) => updateSetting("meeting_min_notice_minutes", event.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-2 text-xs"><option value="30">30 min</option><option value="60">1 hour</option><option value="1440">1 day</option><option value="2880">2 days</option></select></label></div><label className="mt-3 flex items-center justify-between rounded-lg border border-slate-200 px-3 py-2 text-xs font-bold"><span>Accept online meetings</span><input type="checkbox" checked={settingsDraft?.meeting_enabled !== false} onChange={(event) => updateSetting("meeting_enabled", event.target.checked)} className="h-4 w-4 accent-[#EC008C]" /></label><label className="mt-2 flex items-center justify-between rounded-lg border border-slate-200 px-3 py-2 text-xs font-bold"><span>Require owner confirmation</span><input type="checkbox" checked={settingsDraft?.meeting_requires_approval === true} onChange={(event) => updateSetting("meeting_requires_approval", event.target.checked)} className="h-4 w-4 accent-[#EC008C]" /></label><div className="mt-4 border-t border-slate-200 pt-4"><p className="mb-2 text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Open days · {timeZone}</p><div className="space-y-2">{hoursDraft.map((row) => <div key={row.day_of_week} className="grid grid-cols-[minmax(0,1fr)_78px_78px] items-center gap-2 text-xs"><label className="flex items-center gap-2 font-bold"><input type="checkbox" checked={!row.is_closed} onChange={(event) => updateHour(row.day_of_week, { is_closed: !event.target.checked })} className="h-4 w-4 accent-[#00aeb5]" />{DAY_NAMES[row.day_of_week]}</label><input type="time" value={row.opens_at} disabled={row.is_closed} onChange={(event) => updateHour(row.day_of_week, { opens_at: event.target.value })} className="rounded border border-slate-200 px-1 py-1 text-[11px] disabled:bg-slate-100" /><input type="time" value={row.closes_at} disabled={row.is_closed} onChange={(event) => updateHour(row.day_of_week, { closes_at: event.target.value })} className="rounded border border-slate-200 px-1 py-1 text-[11px] disabled:bg-slate-100" /></div>)}</div></div><button type="button" onClick={saveAvailability} disabled={saving} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-slate-900 px-3 py-2.5 text-xs font-bold text-white hover:bg-[#EC008C] disabled:opacity-40">{saving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />}{saving ? "Saving…" : "Save availability"}</button></section>
      ...
      {showMeetingBooking && selectedCall && <MeetingBookingModal businessId={selectedCall.business_id} conversationId={selectedCall.conversation_id} existingCall={selectedCall} isOwner onClose={() => { setShowMeetingBooking(false); setSelectedCall(null); }} onBooked={async () => { setShowMeetingBooking(false); setSelectedCall(null); setNotice("Meeting saved and participants were notified."); await loadCalendar(true); }} />}
    </div>
  );
}
