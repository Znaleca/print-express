...
test("delivery settings and checkout keep fee calculation server-authoritative", async () => {
  ...
  assert.match(ownerShop, /step="any"[^>]*name="delivery_max_distance_km"/);
  assert.match(ownerShop, /onBlur=\{\(\) => formatDistanceInput\("delivery_max_distance_km"\)\}/);
  assert.equal((ownerShop.match(/<LocationPicker/g) || []).length, 1, "owner shop should render one shared location map");
  assert.match(ownerShop, /onBlur=\{handleAddressLookup\}/);
  assert.match(ownerShop, /open-location-code/);
  assert.match(ownerShop, /QFV5\+Q6V/);
  assert.doesNotMatch(ownerShop, /includedRadiusKm=|maxRadiusKm=/);
  assert.match(ownerShop, /Delivery distance uses the shop pin/);
  assert.match(ownerShop, /Delivery available/);
  assert.match(ownerShop, /Delivery costs/);
  assert.doesNotMatch(locationPicker, /L\.circle|kilometersToMeters|includedRadiusKm|maxRadiusKm/);
  assert.match(ownerShop, /form\.delivery_enabled && \(/);
