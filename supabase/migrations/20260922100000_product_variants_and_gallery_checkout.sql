-- Product variants are stored in the existing specs_json document so current
-- products remain compatible. Checkout still resolves price and stock from the
-- locked service row, never from the customer-provided price.
...
begin
  ...
  loop
    ...
    modifier := 0;
    variant_id := nullif(trim(item_specs->>'variant_id'), '');
    variant_index := null;
    variant_row := null;
    ...
    if coalesce((item->>'is_quoted_checkout')::boolean, false) then
      ...
      if item_type = 'product'
         and jsonb_typeof(coalesce(service_row.specs_json->'variants', '[]'::jsonb)) = 'array'
         and jsonb_array_length(coalesce(service_row.specs_json->'variants', '[]'::jsonb)) > 0 then
        if variant_id is null then
          raise exception 'Choose a product variant before checkout';
        end if;

        select ordinal - 1, value
          into variant_index, variant_row
        from jsonb_array_elements(service_row.specs_json->'variants') with ordinality as variants(value, ordinal)
        where value->>'id' = variant_id;

        if variant_row is null then
          raise exception 'The selected product variant is no longer available';
        end if;

        item_specs := item_specs || jsonb_build_object(
          'variant_id', variant_row->>'id',
          'variant_name', variant_row->>'name',
          'variant_sku', variant_row->>'sku'
        );
        unit_price := coalesce((variant_row->>'price')::numeric, coalesce(service_row.price, 0));
      else
    ...
    if item_type = 'product' then
      if variant_row is not null then
        variant_stock := greatest(0, coalesce((variant_row->>'stock_qty')::integer, 0));
        if variant_stock < quantity then
          raise exception 'Insufficient stock for %', item_name;
        end if;

        updated_variants := jsonb_set(
          coalesce(service_row.specs_json->'variants', '[]'::jsonb),
          array[variant_index::text, 'stock_qty'],
          to_jsonb(variant_stock - quantity),
          true
        );
        update public.services
        set stock_qty = greatest(0, coalesce(stock_qty, 0) - quantity),
            specs_json = jsonb_set(coalesce(specs_json, '{}'::jsonb), '{variants}', updated_variants, true),
            updated_at = now()
      ...
      insert into public.inventory_movements (
        ...
        greatest(0, coalesce(service_row.stock_qty, 0) - quantity),
        'ORDER_DEDUCTION',
        case when variant_row is null then 'Atomic checkout' else 'Atomic checkout · ' || coalesce(variant_row->>'name', 'Product variant') end,
        customer
      );
    end if;
